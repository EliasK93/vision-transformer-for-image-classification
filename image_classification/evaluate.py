...
def evaluate(model_dir, model_name, data_dir):
    ...
    for label in os.listdir(data_dir + "/test/"):
        for img in os.listdir(data_dir + "/test/" + label + "/"):
            ...
            label_pred = classifier.predict_image(image)
            real_labels.append(label)
            pred_labels.append(label_pred)
    pbar.close()

    # use sklearn to evaluate predictions
    classification_report = sklearn.metrics.classification_report(real_labels, pred_labels)
    print(classification_report)


if __name__ == '__main__':
    evaluate(model_dir="./", model_name="ViT-base-224", data_dir="data_augmented/")
