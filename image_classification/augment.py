    ...
    for label, file_list in file_dict.items():
        ...
        for ind, img_name in enumerate(tqdm(file_list, desc="Running augmentation for label " + label)):
            if ind < test_size:
                ...
                set_type = "train"
            img = Image.open(original_dir + label + "/" + img_name).convert('RGB').resize((224, 224))
...
if __name__ == '__main__':
    create_augmented_dataset(original_dir="data/", augmented_dir="data_augmented/", aug_number=5, test_ratio=0.1, seed=1)
